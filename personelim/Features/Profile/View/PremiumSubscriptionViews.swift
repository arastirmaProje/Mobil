...
struct PremiumPromotionCard: View {
    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(ConstantStrings.premiumCardDescription)
                        .font(.system(size: 16))
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(ConstantStrings.premiumTitle)
    }
}
...
struct PremiumSubscriptionView: View {
    ...
    private func purchaseSelectedPlan() async {
        let success = await viewModel.purchaseSelectedPlan(businessId: businessId)
    ...
    var body: some View {
        Button(action: onTap) {
            ZStack(alignment: .top) {
                ...
                .frame(minHeight: 110)
                .frame(maxWidth: .infinity)
                .background(plan.isHighlighted ? Color.blue.opacity(0.18) : Color(.systemGray6))
                .overlay(
                    RoundedRectangle(cornerRadius: 10)
                        .stroke(plan.isHighlighted ? Color.blue : Color.clear, lineWidth: 3)
                )
                .clipShape(RoundedRectangle(cornerRadius: 10))
